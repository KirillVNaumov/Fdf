					...
					t_point *second, int minimum_altitude)
{
	...
	while (++point->y < second->y)
	{
		...
		minimum_altitude += color_increment * 200; //100 500
		d = d + 2 * diff->x;
	}
	free(diff);
	free(point);
...
void	printline(t_fdf *fdf)
{
	...
	else
	{
		if (fdf->first->y > fdf->second->y)
			print_high(fdf, fdf->second, fdf->first, minimum_altitude + 9056567);
		else
			print_high(fdf, fdf->first, fdf->second, minimum_altitude + 9056567);
	}
}
